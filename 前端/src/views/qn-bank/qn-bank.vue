...
<template>
  <div class="Design" v-loading="loading" element-loading-text="加载中...">
    <div class="top">
      <span v-if="isImport">请选择题目</span>
      <el-row v-if="!isImport" :gutter="30">
        <el-col :span="3">
          <el-button icon="el-icon-circle-plus" @click="addQuestion" style="margin-top: 10px;">添加题目</el-button>
        </el-col>
        <el-col :span="3">
          <el-upload action="/api/qnBank/batchUpload" accept=".xls, .xlsx" :show-file-list="false" :before-upload="beforeUploadQn"
            style="padding-top: 12px;width: 117px;" :on-success="getQuestionList">
    ...
    <el-card class="box-card" v-for="(item, index) in detail" :key="index" style="margin: 10px;">
      <div slot="header" class="clearfix">
        ...
        <div style="float: right;">
          <el-button style="padding: 2px" type="text" @click="editorQuestion(item)" v-if="!isImport">编辑</el-button>
          <el-button style="padding: 2px;color: #F56C6C" type="text" @click="deleteQuestion(index)"
            v-if="!isImport">删除</el-button>
          <el-checkbox style="padding: 2px;color: #F56C6C" type="text" v-model="item.checked" v-if="isImport"></el-checkbox>
        </div>
      </div>
      <!--单选题展示-->
      ...
      <!--填空题展示-->
      <el-input v-if="item.type == 'text'" type="textarea" :rows="item.row" resize="none" v-model="item.textValue"></el-input>
    </el-card>
    <br><br><br><br><br>
    <!--添加题目弹窗-->
    <el-dialog :questionTitle="dialogTitle" :visible.sync="dialogShow" :close-on-click-modal="false" class="dialog">
      <el-form ref="form" :model="willAddQuestion" label-width="80px">
        ...
        <el-form-item label="上传图片" required style="width: 100%;">
          <el-upload action="/api/question/image" :show-file-list="false" :before-upload="beforeUploadImage"
            :data="willAddQuestion" :auto-upload="false" ref="uploadImage" :http-request="UploadFile">
