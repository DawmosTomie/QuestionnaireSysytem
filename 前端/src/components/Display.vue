...
<template>
  <div class="display">
    <div class="content">
      ...
      <el-card class="box-card" v-for="(item,index) in detail" :key="item.id">
        ...
        <!--单选题展示-->
        <div class="text item" v-if="item.type=='radio'" v-for="optionItem in item.options" :key="optionItem.id">
          <el-radio v-model="item.radioValue" :label="optionItem.title" style="margin: 5px;">{{ optionItem.title }}</el-radio>
        </div>

        <!--多选题展示-->
        <el-checkbox-group v-if="item.type=='checkbox'" v-model="item.checkboxValue">
          <div class="text item"  v-for="optionItem in item.options" :key="optionItem.id">
...
<script>
  import {answerOpera,queryQuestions} from '../utils/api'
  export default{
    data(){
      return{
        dialogShow:false,
    ...
    mounted(){
      var wjId=this.$route.params.questionnaireId;
       answerOpera({
              opera_type:'get_info',
